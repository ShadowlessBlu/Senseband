...
// Pulse Sensor
#define PULSE_PIN 3 // Analog pin for pulse sensor
PulseSensorPlayground pulseSensor;
const int BPM_MIN = 50; // Minimum normal BPM
const int BPM_MAX = 120; // Maximum normal BPM
...
VibrationPattern bpmPattern = {100, 100, 4};   // Quick bursts for heart rate alert

// Global Variables
bool fallDetected = false;
bool sosTriggered = false;
bool outsideGeofence = false;
bool abnormalBPM = false;
...
void checkPulse() {
  if (pulseSensor.sawStartOfBeat()) {
    ...
    if (bpm < BPM_MIN || bpm > BPM_MAX) {
      abnormalBPM = true;
      lastBpmTime = String(gps.date.year()) + "-" + String(gps.date.month()) + "-" + String(gps.date.day()) + " " +
                    String(gps.time.hour()) + ":" + String(gps.time.minute()) + ":" + String(gps.time.second());
      String message = "Abnormal Heart Rate: " + String(bpm) + " BPM! Location: " +
...
void sendWebSocketData() {
  ...
  json += "\"bpmLog\":\"" + (abnormalBPM ? "Abnormal BPM at " + lastBpmTime : "") + "\",";
...
void setup() {
  ...
  pulseSensor.setThreshold(550); // Adjust based on sensor calibration
...
void loop() {
  ...
  // Sleep Mode
  if (!isMoving && !fallDetected && !sosTriggered && !outsideGeofence && !abnormalBPM) {
